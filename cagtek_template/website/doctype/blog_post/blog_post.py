...
class BlogPost(WebsiteGenerator):
    ...
    def make_route(self):
        if not self.route:
            return harpiya.db.get_value('Blog Category', self.blog_category,
    ...
    def validate(self):
        ...
        if self.featured:
            if not self.meta_image:
                harpiya.throw(_("A featured post must have a cover image"))
            self.reset_featured_for_other_blogs()

        self.set_read_time()

    def reset_featured_for_other_blogs(self):
        all_posts = harpiya.get_all("Blog Post", {"featured": 1})
        for post in all_posts:
            harpiya.db.set_value("Blog Post", post.name, "featured", 0)
    ...
    def get_context(self, context):
        ...
        context.enable_cta = not self.hide_cta and harpiya.db.get_single_value("Blog Settings", "show_cta_in_blog",
                                                                               cache=True)

        if self.blogger:
            context.blogger_info = harpiya.get_doc("Blogger", self.blogger).as_dict()
        ...
        context.category = harpiya.db.get_value("Blog Category",
                                                context.doc.blog_category, ["title", "route"], as_dict=1)
        context.parents = [{"name": _("Home"), "route": "/"},
                           {"name": "Blog", "route": "/blog"},
                           {"label": context.category.title, "route": context.category.route}]
        context.guest_allowed = harpiya.db.get_single_value("Blog Settings", "allow_guest_to_comment")

    def fetch_cta(self):
        if harpiya.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
            blog_settings = harpiya.get_cached_doc("Blog Settings")
    ...
    def fetch_social_links_info(self):
        if not harpiya.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
            return []
    ...
    def load_feedback(self, context):
        ...
        feedback = harpiya.get_all('Feedback',
                                   fields=['like'],
                                   filters=dict(
                                       reference_doctype=self.doctype,
                                       reference_name=self.name,
                                       ip_address=harpiya.local.request_ip,
                                       owner=user
                                   )
                                   )
        ...
        if harpiya.db.count('Feedback'):
            like_count = harpiya.db.count('Feedback',
                                          filters=dict(
                                              reference_doctype=self.doctype,
                                              reference_name=self.name,
                                              like=True
...
def get_list_context(context=None):
    ...
    category = harpiya.utils.escape_html(harpiya.local.form_dict.blog_category or harpiya.local.form_dict.category)
    if category:
    ...
    elif harpiya.local.form_dict.blogger:
        blogger = harpiya.db.get_value("Blogger", {"name": harpiya.local.form_dict.blogger}, "full_name")
    ...
    blog_settings = harpiya.get_doc("Blog Settings").as_dict(no_default_fields=True)
...
def get_blog_categories():
    ...
    post, category = harpiya.qb.DocType("Blog Post"), harpiya.qb.DocType("Blog Category")
    return (
...
def clear_blog_cache():
    for blog in harpiya.db.sql_list("""select route from
...
def get_blog_category(route):
    return harpiya.db.get_value("Blog Category", {"name": route}, "title") or route


def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
    conditions = []
    category = filters.blog_category or harpiya.utils.escape_html(
        harpiya.local.form_dict.blog_category or harpiya.local.form_dict.category)
    if filters:
        if filters.blogger:
            conditions.append('t1.blogger=%s' % harpiya.db.escape(filters.blogger))
    if category:
        conditions.append('t1.blog_category=%s' % harpiya.db.escape(category))
    ...
    for post in posts:
        ...
        post.category = harpiya.db.get_value('Blog Category', post.blog_category,
